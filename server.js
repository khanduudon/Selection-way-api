...
const ENC_KEY  = process.env.ENC_KEY || "12345678901234567890123456789012"; // 32 bytes
...
function sendEncrypted(res, rawPayload) {
  const encrypted = encryptData(rawPayload);
  if (!encrypted) throw new Error("Encryption Logic Failed");
  return res.json({ success: true, payload: encrypted });
}
...
app.get('/api/batches', async (req, res) => {
  ...
  try {
    ...
    sendEncrypted(res, { status: "success", total_batches: data.length, batches: data });
  } catch (e) {
...
app.get('/api/subjects', async (req, res) => {
  ...
  if (!batch_id) return res.status(400).json({ detail: "batch_id is required" });

  const url = `${BASE_URL}/subjects?batchId=${batch_id}&userId=${user_id}&limit=1000`;
  try {
    const json = await externalFetch(url);
    const data = json.data || [];
    sendEncrypted(res, { status: "success", batch_id, total_subjects: data.length, subjects: data });
  } catch (e) {
...
app.get('/api/topics', async (req, res) => {
  ...
  if (!subject_id) return res.status(400).json({ detail: "subject_id is required" });

  const url = `${BASE_URL}/topics?subjectId=${subject_id}&batchId=${batch_id || ''}&userId=${user_id}&limit=1000`;
  try {
    const json = await externalFetch(url);
    const data = json.data || [];
    sendEncrypted(res, { status: "success", subject_id, total_topics: data.length, topics: data });
  } catch (e) {
...
app.get('/api/videos', async (req, res) => {
  const { topic_id, subject_id, batch_id, user_id = "3708939" } = req.query;
  if (!topic_id) return res.status(400).json({ detail: "topic_id is required" });

  const url = `${BASE_URL}/videos?topicId=${topic_id}&subjectId=${subject_id || ''}&batchId=${batch_id || ''}&userId=${user_id}&limit=1000`;
  try {
    const json = await externalFetch(url);
    const data = json.data || [];
    sendEncrypted(res, { status: "success", topic_id, total_videos: data.length, videos: data });
  } catch (e) {
...
app.get('/api/pdfs', async (req, res) => {
  ...
  const url = `${BASE_URL}/pdfs?topicId=${topic_id}&subjectId=${subject_id || ''}&batchId=${batch_id || ''}&userId=${user_id}&limit=1000`;
  try {
    const json = await externalFetch(url);
    const data = json.data || [];
    sendEncrypted(res, { status: "success", topic_id, total_pdfs: data.length, pdfs: data });
  } catch (e) {
...
// ─── 404 FALLBACK ─────────────────────────────────────────────────────────────
app.use((req, res) => {
  res.status(404).json({ detail: "Route not found bhai 😅" });
});
